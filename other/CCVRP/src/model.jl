struct CumulativeResArcParams
    t::Cdouble # time consumption
    w::Cdouble # weight
end

struct CumulativeResVertexParams end

struct CumulativeResConstParams
    Wmax::Cdouble # maximum accumulated weight
    Tmax::Cdouble # maximum accumulated time consumption
end
...
function build_model(data::DataCVRP, app::Dict{String,Any})
    ...
    @variable(cvrp.formulation, z >= 0)
    @objective(cvrp.formulation, Min, z)
    ...
    function build_graph()
        ...
        Wmax = Cdouble(Q)
        Tmax = Cdouble(app["ub"] / minimum(d(data, i) for i in V⁺ if d(data, i) > 0.0))
        ...
        for (i, j) in E
            ...
            t_ij = Cdouble(d(data, j))
            t_ji = Cdouble(d(data, i))
            w_ij = Cdouble(c(data, (i, j)))
            add_arc_var_mapping!(G, arc_id, x[(i, j)])
            set_arc_consumption!(G, arc_id, cap_res_id, (d(data, i) + d(data, j)) / 2)
            set_arc_custom_res_params!(G, arc_id, cum_res_id, CumulativeResArcParams(w_ij, t_ij))
            arc_id = add_arc!(G, j, i)
            add_arc_var_mapping!(G, arc_id, x[(i, j)])
            set_arc_consumption!(G, arc_id, cap_res_id, (d(data, i) + d(data, j)) / 2)
            set_arc_custom_res_params!(G, arc_id, cum_res_id, CumulativeResArcParams(w_ij, t_ji))
        end
